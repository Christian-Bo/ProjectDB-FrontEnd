<%--
  Cuentas por Cobrar (UNIFICADO)
  - Listado + filtros + paginación
  - Modal: Registrar pago
  - Modal: Detalle de documento (antes cxc_detalle.jsp)
  - Modal: Pagos registrados (antes cxc_pagos.jsp)
--%>
...
<body class="nt-bg min-vh-100 d-flex flex-column">
  ...
  <!-- Modal: Registrar Pago (ya existía) -->
  ...
  <!-- Modal: Detalle de CxC (antes cxc_detalle.jsp) -->
  ...
  // Helpers de navegación como en otras vistas
  ...
  const API_CXC  = API_BASE + '/api/cxc';
  const API_CAT  = API_BASE + '/api/catalogos';
  const CTX      = '${pageContext.request.contextPath}';

  let page = 0, size = 10, last = {};
  let _clienteId = null;
  let _docPago   = { id:null, numero:null, saldo:0, clienteId:null };

  const CLIENTES_MAP = new Map();  // id -> "COD - NOMBRE"
  ...
  function clienteLabel(cli){
    ...
    return (codigo && nombre) ? (codigo + ' - ' + nombre) : (nombre || ('Cliente ID ' + (cli.id ?? '')));
  }
  ...
  async function cargar(params){
    ...
    try{
      ...
      if (clienteId){
        if (params.estado){ rows = rows.filter(x => (x.estado||'') === params.estado); }
        if (params.desde){ rows = rows.filter(x => !x.fechaEmision || x.fechaEmision >= params.desde); }
        if (params.hasta){ rows = rows.filter(x => !x.fechaEmision || x.fechaEmision <= params.hasta); }
      ...
      tr.innerHTML =
        ...
        '<td class="text-end">' +
          '<div class="btn-group btn-group-sm">' +
            '<button class="btn btn-outline-secondary" onclick="verDetalle(' + id + ')">Ver</button>' +
  ...
  async function verDetalle(documentoId){
    ...
    try{
      ...
      const [doc, apl] = await Promise.all([ fetchJson(urlDoc), fetchJson(urlApl) ]);
      ...
      const abonado = aplArr.reduce((s,a)=> s + Number(a.montoAplicado||0), 0);
  ...
  async function cargarPagosFlat(params){
    ...
    try{
      ...
      // Consultar aplicaciones por pago y aplanar
      const combos = await Promise.all(
        pagosArr.map(p =>
          fetchJson(API_CXC + '/pagos/' + encodeURIComponent(p.pagoId) + '/aplicaciones')
            .then(apps => ({ pago: p, apps: (Array.isArray(apps)? apps : []) }))
            .catch(()=>({ pago: p, apps: [] }))
        )
      );

      const flat = [];
      combos.forEach(({pago, apps})=>{
        ...
        apps.forEach(a=>{
          flat.push({
            ...
            documento:       a.numeroDocumento || ('ID ' + (a.documentoId ?? '')),
      ...
      renderPagosFlat(flat);
    }catch(err){
      alert('Error cargando pagos: ' + err.message);
      renderPagosFlat([]);
    }
  }
  function renderPagosFlat(rows){
  ...
  function toggleExtras(){
    const forma = (document.getElementById('pForma')?.value || '').toUpperCase();
  ...
  function cardLast4(){
    const raw = (document.getElementById('cardNum')?.value || '').replace(/\D/g,'');
    return raw.slice(-4);
  }
  function buildExtraObservaciones(forma){
    ...
    if (forma === 'TRF'){
      const bank = (document.getElementById('trfBank')?.value || '').trim();
      const ref  = (document.getElementById('trfRef')?.value  || '').trim();
      const fdep = (document.getElementById('trfDate')?.value || '').trim();
  ...
  function abrirPago(docId, docNum, saldo, cliId){
    ...
    ['cardNum','cardExp','cardCvv','cardName','trfBank','trfRef'].forEach(id=>{
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    cargarClientes(_clienteId).then(function(){
      cargar({ clienteId: _clienteId || '' });
      // precargar tabla de pagos al abrir modal
      document.getElementById('mdlPagos').addEventListener('shown.bs.modal', function(){
        // Usar selección (si hay) para prefiltrar
        const pre = document.getElementById('selCliente')?.value || '';
        if (pre) document.getElementById('selClientePagos').value = pre;
    ...
    if (btnCobrar){
      btnCobrar.addEventListener('click', async function(){
        try{
          if (!_clienteId) throw new Error('Selecciona un cliente antes de registrar pagos.');
          ...
          if (forma === 'TAR'){
            ...
            if (!vCard.ok){ alert(vCard.msg); return; }
          }
          if (forma === 'TRF'){
            const bank = (document.getElementById('trfBank')?.value || '').trim();
            const ref  = (document.getElementById('trfRef')?.value  || '').trim();
            if (!bank && !ref){ alert('Para transferencia indique banco o referencia.'); return; }
          }
          ...
          alert('Pago aplicado. ID ' + crearRes.pagoId);
          cargar(last);
        }catch(err){
